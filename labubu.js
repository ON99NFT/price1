const labubu = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'labubu-audio-enable-btn';
        ...
        const section = document.getElementById('labubu-buy-alert').closest('.token-section');
...
async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/LABUBU_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetLABUBU) => {
            let totalLABUBU = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const labubuAvailable = usdtAvailable / price;
                const remaining = targetLABUBU - totalLABUBU;
                const fillAmount = Math.min(remaining, labubuAvailable);
                
                totalUSDT += fillAmount * price;
                totalLABUBU += fillAmount;
                
                if (totalLABUBU >= targetLABUBU) break;
            }
            if (totalLABUBU < targetLABUBU) throw new Error('Insufficient bid liquidity');
            return totalUSDT / targetLABUBU;
        };

        const calculateAskPrice = (asks, targetLABUBU) => {
            let totalLABUBU = 0;
            let totalUSDT = 0;
            ...
            if (totalLABUBU < targetLABUBU) throw new Error('Insufficient ask liquidity');
            return totalUSDT / targetLABUBU;
        };

        const targetLABUBU = 5249; // Matches JUP's sell amount
        const bidPrice = calculateBidPrice(data.data.bids, targetLABUBU);
        const askPrice = calculateAskPrice(data.data.asks, targetLABUBU);
    ...
    async function fetchJupPrice() {
        ...
        const outputMintlabubu = 'JB2wezZLdzWfnaCfHxLg193RS3Rh51ThiXxEDWQDpump';
    
        const [labubuAmount, usdcAmount] = await Promise.all([
            fetchJupSwapPrice(inputMintUSDC, outputMintlabubu, 298 * 1e6, 6),
            fetchJupSwapPrice(outputMintlabubu, inputMintUSDC, 5249 * 1e6, 6)
        ]);
    
        return {
            buyPrice: labubuAmount ? 298 / labubuAmount : null,
            sellPrice: usdcAmount ? usdcAmount / 5249 : null
        };
    }
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('labubu-buy-alert'),
            sell: document.getElementById('labubu-sell-alert')
        };
        ...
        try {
            ...
            // Update labubuplay with price comparison
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'labubu-buy-alert') {
            playAlertSound();
        }
    }

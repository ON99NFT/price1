const troll = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'troll-audio-enable-btn';
        ...
        const section = document.getElementById('troll-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // TROLL received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/TROLLSOL_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetTROLL) => {
            let totalTROLL = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const trollAvailable = usdtAvailable / price;
                const remaining = targetTROLL - totalTROLL;
                const fillAmount = Math.min(remaining, trollAvailable);
                
                totalUSDT += fillAmount * price;
                totalTROLL += fillAmount;
                
                if (totalTROLL >= targetTROLL) break;
            }
            return totalUSDT / targetTROLL;
        };

        const calculateAskPrice = (asks, targetTROLL) => {
            let totalTROLL = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const TROLL_DECIMALS = 6;
  
      // Get USDC needed to buy 24999 TROLL
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          TROLL_MINT,
          24999 * 10 ** TROLL_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 24999 TROLL
      ...
      return {
          buyPrice: usdcNeeded / 24999,  // USDC per TROLL (buy)
          sellPrice: usdcReceived / 24999 // USDC per TROLL (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('troll-buy-alert');
      const sellElement = document.getElementById('troll-sell-alert');

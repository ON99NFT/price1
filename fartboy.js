const fartboy = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'fartboy-audio-enable-btn';
        ...
        const section = document.getElementById('fartboy-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/FARTBOY_USDT';
    ...
    async function fetchJupPrice() {
        ...
        const outputMintfartboy = 'y1AZt42vceCmStjW4zetK3VoNarC1VxJ5iDjpiupump';
    
        const [fartboyAmount, usdcAmount] = await Promise.all([
            fetchJupSwapPrice(inputMintUSDC, outputMintfartboy, 500 * 1e6, 9),
            fetchJupSwapPrice(outputMintfartboy, inputMintUSDC, 4500 * 1e9, 6)
        ]);
    
        if (!fartboyAmount || !usdcAmount) return null;
    
        return {
            rateFor500USDC: 500 / fartboyAmount,
            rateFor4500fartboy: usdcAmount / 4500
        };
    }

    // Alert update
    async function updateAlerts() {
        const buyElement = document.getElementById('fartboy-buy-alert');
        const sellElement = document.getElementById('fartboy-sell-alert');
        ...
        try {
            ...
            const sellDiff = (jupData.rateFor4500fartboy - mexcData.ask).toFixed(5);
    ...
    function applyAlertStyles(element, difference) {
        ...
        if (playSound && audioEnabled && element.id === 'fartboy-buy-alert') {
            playAlertSound();
        }
    }

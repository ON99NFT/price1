const startup = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'startup-audio-enable-btn';
        ...
        const section = document.getElementById('startup-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // STARTUP received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/STARTUP_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetSTARTUP) => {
            let totalSTARTUP = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const startupAvailable = usdtAvailable / price;
                const remaining = targetSTARTUP - totalSTARTUP;
                const fillAmount = Math.min(remaining, startupAvailable);
                
                totalUSDT += fillAmount * price;
                totalSTARTUP += fillAmount;
                
                if (totalSTARTUP >= targetSTARTUP) break;
            }
            return totalUSDT / targetSTARTUP;
        };

        const calculateAskPrice = (asks, targetSTARTUP) => {
            let totalSTARTUP = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const STARTUP_MINT = '97PVGU2DzFqsAWaYU17ZBqGvQFmkqtdMywYBNPAfy8vy';
      const STARTUP_DECIMALS = 9;
  
      // Get USDC needed to buy 34998 STARTUP
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          STARTUP_MINT,
          34998 * 10 ** STARTUP_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 34998 STARTUP
      ...
      return {
          buyPrice: usdcNeeded / 34998,  // USDC per STARTUP (buy)
          sellPrice: usdcReceived / 34998 // USDC per STARTUP (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('startup-buy-alert');
      const sellElement = document.getElementById('startup-sell-alert');

const mirai = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'mirai-audio-enable-btn';
        ...
        const section = document.getElementById('mirai-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/MIRAI_USDT';
    ...
    async function fetchJupPrice() {
        ...
        const outputMintmirai = '5evN2exivZXJfLaA1KhHfiJKWfwH8znqyH36w1SFz89Y';
    
        const [miraiAmount, usdcAmount] = await Promise.all([
            fetchJupSwapPrice(inputMintUSDC, outputMintmirai, 598 * 1e6, 6),
            fetchJupSwapPrice(outputMintmirai, inputMintUSDC, 26998 * 1e6, 6)
        ]);
    
        if (!miraiAmount || !usdcAmount) return null;
    
        return {
            rateFor500USDC: 598 / miraiAmount,
            rateFor4500mirai: usdcAmount / 26998
        };
    }

    // Alert update
    async function updateAlerts() {
        const buyElement = document.getElementById('mirai-buy-alert');
        const sellElement = document.getElementById('mirai-sell-alert');
        ...
        try {
            ...
            const sellDiff = (jupData.rateFor4500mirai - mexcData.ask).toFixed(5);
    ...
    function applyAlertStyles(element, difference) {
        ...
        if (playSound && audioEnabled && element.id === 'mirai-buy-alert') {
            playAlertSound();
        }
    }

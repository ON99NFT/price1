const TAT = (() => {
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/TAT_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.TAT, 599 * 1e18),
                fetchKyberSwapPrice(addresses.TAT, addresses.USDT, 44999 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 599 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 44999 : null
            };
        } catch (error) {

const BR = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'br-audio-enable-btn';
        ...
        const section = document.getElementById('br-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BR_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BR, 1699 * 1e18),
                fetchKyberSwapPrice(addresses.BR, addresses.USDT, 13750 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 1699 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 13750 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('br-buy-alert'),
            sell: document.getElementById('br-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'br-buy-alert') {
            playSystemAlert();
        }
    }

const AFT = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'aft-audio-enable-btn';
        ...
        const section = document.getElementById('aft-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/AFT_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.AFT, 799 * 1e18),
                fetchKyberSwapPrice(addresses.AFT, addresses.USDT, 8999 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 799 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 8999 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('aft-buy-alert'),
            sell: document.getElementById('aft-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'aft-buy-alert') {
            playSystemAlert();
        }
    }

  ...
  // Keep the rest of the code the same as previous version
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BROWNIE_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            BROWNIE: '0xAA7cD678C8a2809c6DFF1250a87f184E779922c9'
        };
    
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BROWNIE, 750 * 1e18),
                fetchKyberSwapPrice(addresses.BROWNIE, addresses.USDT, 550000 * 1e18)
            ]);
        
            return {
                buyPrice: buyAmount ? 750 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 550000 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('brownie-buy-alert'),
            sell: document.getElementById('brownie-sell-alert')
        };

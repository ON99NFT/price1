const libra = (() => {
  ...
  function handleAudioInitialization() {
      ...
      enableButton.id = 'libra-audio-enable-btn';
      ...
      const section = document.getElementById('libra-buy-alert').closest('.token-section');
  ...
  // Fixed: Added missing JUP swap function
  ...
  async function fetchJupPrice() {
      ...
      const [libraAmount, usdcAmount] = await Promise.all([
          fetchJupSwapPrice(inputMintUSDC, outputMintlibra, 500 * 1e6, 6),
          fetchJupSwapPrice(outputMintlibra, inputMintUSDC, 1675 * 1e6, 6)
      ]);
  
      if (!libraAmount || !usdcAmount) return null;
  
      return {
          rateFor500USDC: 500 / libraAmount,
          rateFor1675libra: usdcAmount / 1675
      };
  }

  // Update alerts
  async function updateAlerts() {
      const buyElement = document.getElementById('libra-buy-alert');
      const sellElement = document.getElementById('libra-sell-alert');
      ...
      try {
          ...
          const sellDiff = jupData.rateFor1675libra - mexcData.ask;
  ...
  function applyAlertStyles(element, difference) {
      ...
      if (playSound && audioEnabled && element.id === 'libra-buy-alert') {
          playAlertSound();
      }
  }

const gork = (() => {
    ...
    async function fetchMexcPrice() {
        try {
            const data = await fetchWithProxy('https://contract.mexc.com/api/v1/contract/depth/GORK_USDT');
    ...
    async function fetchJupPrice() {
        ...
        const [usdcNeeded, usdcReceived] = await Promise.all([
            fetchJupSwapPrice(USDC_MINT, GORK_MINT, 19998 * 10 ** GORK_DECIMALS, 6, true),
            fetchJupSwapPrice(GORK_MINT, USDC_MINT, 19998 * 10 ** GORK_DECIMALS, 6)
        ]);

        if (!usdcNeeded || !usdcReceived) return null;

        return {
            buyPrice: usdcNeeded / 19998,
            sellPrice: usdcReceived / 19998
        };
    }

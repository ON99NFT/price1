const useless = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'useless-audio-enable-btn';
        ...
        const section = document.getElementById('useless-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // USELESS received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/USELESS_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetUSELESS) => {
            let totalUSELESS = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const uselessAvailable = usdtAvailable / price;
                const remaining = targetUSELESS - totalUSELESS;
                const fillAmount = Math.min(remaining, uselessAvailable);
                
                totalUSDT += fillAmount * price;
                totalUSELESS += fillAmount;
                
                if (totalUSELESS >= targetUSELESS) break;
            }
            return totalUSDT / targetUSELESS;
        };

        const calculateAskPrice = (asks, targetUSELESS) => {
            let totalUSELESS = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const USELESS_DECIMALS = 6;
  
      // Get USDC needed to buy 17498 USELESS
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          USELESS_MINT,
          17498 * 10 ** USELESS_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 17498 USELESS
      ...
      return {
          buyPrice: usdcNeeded / 17498,  // USDC per USELESS (buy)
          sellPrice: usdcReceived / 17498 // USDC per USELESS (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('useless-buy-alert');
      const sellElement = document.getElementById('useless-sell-alert');

const dark = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'dark-audio-enable-btn';
        ...
        const section = document.getElementById('dark-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // DARK received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/DARK_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetDARK) => {
            let totalDARK = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const darkAvailable = usdtAvailable / price;
                const remaining = targetDARK - totalDARK;
                const fillAmount = Math.min(remaining, darkAvailable);
                
                totalUSDT += fillAmount * price;
                totalDARK += fillAmount;
                
                if (totalDARK >= targetDARK) break;
            }
            return totalUSDT / targetDARK;
        };

        const calculateAskPrice = (asks, targetDARK) => {
            let totalDARK = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const DARK_MINT = '8BtoThi2ZoXnF7QQK1Wjmh2JuBw9FjVvhnGMVZ2vpump';
      const DARK_DECIMALS = 6;
  
      // Get USDC needed to buy 34999 DARK
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          DARK_MINT,
          34999 * 10 ** DARK_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 34999 DARK
      ...
      return {
          buyPrice: usdcNeeded / 34999,  // USDC per DARK (buy)
          sellPrice: usdcReceived / 34999 // USDC per DARK (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('dark-buy-alert');
      const sellElement = document.getElementById('dark-sell-alert');

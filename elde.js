const ELDE = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'elde-audio-enable-btn';
        ...
        const section = document.getElementById('elde-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/ELDE_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.ELDE, 498 * 1e18),
                fetchKyberSwapPrice(addresses.ELDE, addresses.USDT, 2498 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 498 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 2498 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('elde-buy-alert'),
            sell: document.getElementById('elde-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'elde-buy-alert') {
            playSystemAlert();
        }
    }

const H = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'h-audio-enable-btn';
        ...
        const section = document.getElementById('h-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/H_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.H, 198 * 1e18),
                fetchKyberSwapPrice(addresses.H, addresses.USDT, 2798 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 198 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 2798 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('h-buy-alert'),
            sell: document.getElementById('h-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        const isBuyAlert = element.parentElement.id === 'h-buy-alert';

const DONKEY = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'donkey-audio-enable-btn';
        ...
        const section = document.getElementById('donkey-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/DONKEY_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.DONKEY, 498 * 1e18),
                fetchKyberSwapPrice(addresses.DONKEY, addresses.USDT, 36998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 498 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 36998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('donkey-buy-alert'),
            sell: document.getElementById('donkey-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'donkey-buy-alert') {
            playSystemAlert();
        }
    }

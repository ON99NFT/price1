// mon.js - Hyperliquid vs MEXC Future comparison for MON
const MON = (() => {
    let audioEnabled = false;
    let hyperliquidPrice = null;
    let mexcPrice = null;

    // Fetch MEXC Futures prices for MON
    async function fetchMexcFuturePrice() {
        const proxyUrl = 'https://api.codetabs.com/v1/proxy/?quest=';
        const url = 'https://contract.mexc.com/api/v1/contract/depth/MONAD_USDT';
        ...
        try {
            ...
            console.error('MEXC Futures MON Error:', error);
            return null;
        }
    }

    // Fetch Hyperliquid Futures prices for MON
        ...
        return new Promise((resolve, reject) => {
            ...
            ws.onopen = () => {
                ws.send(JSON.stringify({
                    method: "subscribe",
                    subscription: { type: "l2Book", coin: "MON" }
                }));
    ...
    async function updateAlerts() {
        const elements = {
            hyperMexcBuy: document.getElementById('mon-hyper-mexc-buy-alert'),
            hyperMexcSell: document.getElementById('mon-hyper-mexc-sell-alert')
        };
        ...
        try {
            ...
            const [hyperData, mexcFutureData] = await Promise.all([
                fetchHyperliquidFuturePrice().catch(error => {
                    console.error('Hyperliquid MON Error:', error);
                    return null;
                }),
                fetchMexcFuturePrice().catch(error => {
                    console.error('MEXC Future MON Error:', error);
                    return null;
                })
            ]);
            ...
            console.error('MON Update Error:', error);

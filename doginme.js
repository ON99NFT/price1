const DOGINME = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'doginme-audio-enable-btn';
        ...
        const section = document.getElementById('doginme-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/DOGINME_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDC, addresses.DOGINME, 599 * 1e6),
                fetchKyberSwapPrice(addresses.DOGINME, addresses.USDC, 499999 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 599 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e6) / 499999 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('doginme-buy-alert'),
            sell: document.getElementById('doginme-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'doginme-buy-alert') {
            playSystemAlert();
        }
    }

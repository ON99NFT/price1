const broccoli1 = (() => {
    ...
    async function fetchMexcSpotPrice() {
      try {
        const proxyUrl = 'https://api.codetabs.com/v1/proxy/?quest=';
        const apiUrl = 'https://api.mexc.com/api/v3/depth?symbol=BROCCOLIUSDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        ...
        return { 
          rateFor4000USDC: bestAsk,  // Use ask price for buy comparison
          rateFor27500broccoli1: bestBid   // Use bid price for sell comparison
        };
      } catch (error) {
    ...
    async function updateAlerts() {
      const buyAlertElement = document.getElementById('broccoli1-buy-alert');
      const sellAlertElement = document.getElementById('broccoli1-sell-alert');
    
      const mexcPrices = await fetchMexcPrice();
      const mexcSpotPrices = await fetchMexcSpotPrice();
    
      if (mexcPrices !== null && mexcSpotPrices !== null) {
        const buyDifference = mexcPrices.bid - mexcSpotPrices.rateFor4000USDC;
        buyAlertElement.textContent = buyDifference.toFixed(5);
        applyAlertStyles(buyAlertElement, buyDifference);
    
        const sellDifference = mexcSpotPrices.rateFor27500broccoli1 - mexcPrices.ask;

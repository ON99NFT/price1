const jailstool = (() => {
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // JAILSTOOL received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const calculateBidPrice = (bids, targetJAILSTOOL) => {
            let totalJAILSTOOL = 0;
            let totalUSDT = 0;
            
            for (const [priceStr, usdtAvailableStr] of bids) {
                const price = parseFloat(priceStr);
                const usdtAvailable = parseFloat(usdtAvailableStr);
                const jailstoolAvailable = usdtAvailable / price;
                const remaining = targetJAILSTOOL - totalJAILSTOOL;
                const fillAmount = Math.min(remaining, jailstoolAvailable);
                
                totalUSDT += fillAmount * price;
                totalJAILSTOOL += fillAmount;
                
                if (totalJAILSTOOL >= targetJAILSTOOL) break;
            }
            return totalUSDT / targetJAILSTOOL;
        };

        const calculateAskPrice = (asks, targetJAILSTOOL) => {
            let totalJAILSTOOL = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const JAILSTOOL_MINT = 'AxriehR6Xw3adzHopnvMn7GcpRFcD41ddpiTWMg6pump';
      const JAILSTOOL_DECIMALS = 6;
  
      // Get USDC needed to buy 100000 JAILSTOOL
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          JAILSTOOL_MINT,
          100000 * 10 ** JAILSTOOL_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 100000 JAILSTOOL
      ...
      return {
          buyPrice: usdcNeeded / 100000,  // USDC per JAILSTOOL (buy)
          sellPrice: usdcReceived / 100000 // USDC per JAILSTOOL (sell)
      };
  }

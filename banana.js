const BANANA = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'banana-audio-enable-btn';
        ...
        const section = document.getElementById('banana-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BANANAS31_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BANANA, 999 * 1e18),
                fetchKyberSwapPrice(addresses.BANANA, addresses.USDT, 249999 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 999 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 249999 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('banana-buy-alert'),
            sell: document.getElementById('banana-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'banana-buy-alert') {
            playSystemAlert();
        }
    }

const HOMEBASE = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'homebase-audio-enable-btn';
        ...
        const section = document.getElementById('homebase-buy-alert').closest('.token-section');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDC, addresses.HOME, 298 * 1e6),
                fetchKyberSwapPrice(addresses.HOME, addresses.USDC, 8498 * 1e18)
            ]);
  
            return {
                buyPrice: buyAmount ? 298 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e6) / 8498 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('homebase-buy-alert'),
            sell: document.getElementById('homebase-sell-alert')
        };
...
function applyAlertStyles(element, value) {
    ...
    const isBuyAlert = element.parentElement.id === 'homebase-buy-alert';

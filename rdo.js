const RDO = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'rdo-audio-enable-btn';
        ...
        const section = document.getElementById('rdo-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/RDO_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.RDO, 498 * 1e18),
                fetchKyberSwapPrice(addresses.RDO, addresses.USDT, 54998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 498 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 54998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('rdo-buy-alert'),
            sell: document.getElementById('rdo-sell-alert')
        };

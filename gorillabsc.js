const GORILLABSC = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'gorillabsc-audio-enable-btn';
        ...
        const section = document.getElementById('gorillabsc-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/GORILLABSC_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            GORILLABSC: '0xcF640FDF9b3d9E45cbd69fDA91D7e22579c14444'
        };

        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.GORILLABSC, 498 * 1e18),
                fetchKyberSwapPrice(addresses.GORILLABSC, addresses.USDT, 44998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 498 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 44998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('gorillabsc-buy-alert'),
            sell: document.getElementById('gorillabsc-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'gorillabsc-buy-alert') {
            playSystemAlert();
        }
    }

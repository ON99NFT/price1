const gork = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'gork-audio-enable-btn';
        ...
        const section = document.getElementById('gork-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // GORK received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/GORK_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetGORK) => {
            let totalGORK = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const gorkAvailable = usdtAvailable / price;
                const remaining = targetGORK - totalGORK;
                const fillAmount = Math.min(remaining, gorkAvailable);
                
                totalUSDT += fillAmount * price;
                totalGORK += fillAmount;
                
                if (totalGORK >= targetGORK) break;
            }
            return totalUSDT / targetGORK;
        };

        const calculateAskPrice = (asks, targetGORK) => {
            let totalGORK = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      // Get USDC needed to buy 9998 GORK
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          GORK_MINT,
          9998 * 10 ** GORK_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 9998 GORK
      ...
      return {
          buyPrice: usdcNeeded / 9998,  // USDC per GORK (buy)
          sellPrice: usdcReceived / 9998 // USDC per GORK (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('gork-buy-alert');
      const sellElement = document.getElementById('gork-sell-alert');

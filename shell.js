  ...
  // Keep the rest of the code the same as previous version
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/SHELL_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            SHELL: '0xf2c88757f8d03634671208935974B60a2a28Bdb3'
        };
    
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.SHELL, 750 * 1e18),
                fetchKyberSwapPrice(addresses.SHELL, addresses.USDT, 1675 * 1e18)
            ]);
        
            return {
                buyPrice: buyAmount ? 750 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 1675 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('shell-buy-alert'),
            sell: document.getElementById('shell-sell-alert')
        };

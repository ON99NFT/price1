const TAG = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'tag-audio-enable-btn';
        ...
        const section = document.getElementById('tag-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/TAG_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.TAG, 598 * 1e18),
                fetchKyberSwapPrice(addresses.TAG, addresses.USDT, 1499998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 598 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 1499998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('tag-buy-alert'),
            sell: document.getElementById('tag-sell-alert')
        };

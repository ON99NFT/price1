const JAGER = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'jager-audio-enable-btn';
        ...
        const section = document.getElementById('jager-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/JAGER_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.JAGER, 498 * 1e18),
                fetchKyberSwapPrice(addresses.JAGER, addresses.USDT, 249999999998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 498 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 249999999998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('jager-buy-alert'),
            sell: document.getElementById('jager-sell-alert')
        };
        ...
        try {
            ...
            const formatPrice = (val) => isNaN(val) ? 'N/A' : val.toFixed(12);
            const formatDiff = (val) => isNaN(val) ? 'N/A' : val.toFixed(12);
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'jager-buy-alert') {
            playSystemAlert();
        }
    }

const house = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'house-audio-enable-btn';
        ...
        const section = document.getElementById('house-buy-alert').closest('.token-section');
    ...
    async function fetchJupSwapPrice(inputMint, outputMint, amount, decimals, exactOut = false) {
      try {
          ...
          return exactOut 
              ...
              : data.outAmount / 10 ** decimals; // HOUSE received for ExactIn
      } catch (error) {
  ...
  async function fetchMexcPrice() {
    try {
        ...
        const apiUrl = 'https://contract.mexc.com/api/v1/contract/depth/HOUSE_USDT';
        const response = await fetch(proxyUrl + apiUrl);
        const data = await response.json();
        
        const calculateBidPrice = (bids, targetHOUSE) => {
            let totalHOUSE = 0;
            let totalUSDT = 0;
            ...
            for (const [priceStr, usdtAvailableStr] of bids) {
                ...
                const houseAvailable = usdtAvailable / price;
                const remaining = targetHOUSE - totalHOUSE;
                const fillAmount = Math.min(remaining, houseAvailable);
                
                totalUSDT += fillAmount * price;
                totalHOUSE += fillAmount;
                
                if (totalHOUSE >= targetHOUSE) break;
            }
            return totalUSDT / targetHOUSE;
        };

        const calculateAskPrice = (asks, targetHOUSE) => {
            let totalHOUSE = 0;
            let totalUSDT = 0;
  ...
  async function fetchJupPrice() {
      ...
      const HOUSE_MINT = 'DitHyRMQiSDhn5cnKMJV2CDDt6sVct96YrECiM49pump';
      const HOUSE_DECIMALS = 6;
  
      // Get USDC needed to buy 9999 HOUSE
      const usdcNeeded = await fetchJupSwapPrice(
          USDC_MINT,
          HOUSE_MINT,
          9999 * 10 ** HOUSE_DECIMALS,
          6,
          true
      );
  
      // Get USDC received for selling 9999 HOUSE
      ...
      return {
          buyPrice: usdcNeeded / 9999,  // USDC per HOUSE (buy)
          sellPrice: usdcReceived / 9999 // USDC per HOUSE (sell)
      };
  }
  
  // Updated alert display
  async function updateAlerts() {
      const buyElement = document.getElementById('house-buy-alert');
      const sellElement = document.getElementById('house-sell-alert');

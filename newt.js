...
function createAudioEnableButton(tokenId) {
    ...
    async function fetchMexcContractPrice() {
        ...
        const url = 'https://contract.mexc.com/api/v1/contract/depth/NEWT_USDT';
    ...
    async function fetchMexcSpotPrice() {
        const proxyUrl = 'https://api.codetabs.com/v1/proxy/?quest=';
        const url = 'https://api.mexc.com/api/v3/depth?symbol=NEWTUSDT&limit=5';
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            NEWT: '0xb8a677e6d805c8d743e6f14c8bc9c19305b5defc'
        };

        try {
            // Format amounts properly without scientific notation
            const buyAmount = "198000000000000000000"; // 198 USDT with 18 decimals
            const sellAmount = "2798000000000000000000"; // 2798 NEWT with 18 decimals
            
            const [buyResponse, sellResponse] = await Promise.all([
                fetch(`https://aggregator-api.kyberswap.com/bsc/api/v1/routes?tokenIn=${addresses.USDT}&tokenOut=${addresses.NEWT}&amountIn=${buyAmount}`),
                fetch(`https://aggregator-api.kyberswap.com/bsc/api/v1/routes?tokenIn=${addresses.NEWT}&tokenOut=${addresses.USDT}&amountIn=${sellAmount}`)
            ]);
    ...
    async function updateAlerts() {
        const elements = {
            kyberBuy: document.getElementById('newt-kyber-buy-alert'),
            kyberSell: document.getElementById('newt-kyber-sell-alert'),
            mexcBuy: document.getElementById('newt-mexc-buy-alert'),
            mexcSell: document.getElementById('newt-mexc-sell-alert')
        };
    ...
    (function init() {
        ...
        setTimeout(() => {
            if (!audioEnabled) {
                // NEW: Check only within NEWT's token section
                const section = document.getElementById('newt-kyber-buy-alert')?.closest('.token-section') || 
                                document.getElementById('newt-buy-alert')?.closest('.token-section');
                
                // Create button only if section exists and has no audio button
                if (section && !section.querySelector('.audio-btn-container')) {
                    createAudioEnableButton('newt');
                }
            }
        }, 5000);
    })();

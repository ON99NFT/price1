const NEWT = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'newt-audio-enable-btn';
        ...
        const section = document.getElementById('newt-buy-alert').closest('.token-section');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.NEWT, 398 * 1e18),
                fetchKyberSwapPrice(addresses.NEWT, addresses.USDT, 798 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 398 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 798 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('newt-buy-alert'),
            sell: document.getElementById('newt-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        const isBuyAlert = element.parentElement.id === 'newt-buy-alert';

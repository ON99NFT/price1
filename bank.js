const BANK = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'bank-audio-enable-btn';
        ...
        const section = document.getElementById('bank-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BANK_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BANK, 799 * 1e18),
                fetchKyberSwapPrice(addresses.BANK, addresses.USDT, 42499 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 799 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 42499 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('bank-buy-alert'),
            sell: document.getElementById('bank-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'bank-buy-alert') {
            playSystemAlert();
        }
    }

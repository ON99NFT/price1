const SPK = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'spk-audio-enable-btn';
        ...
        const section = document.getElementById('spk-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/SPK_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            SPK: '0xaff2e841851700d1fc101995ee6b81ae21bb87d7'
        };

        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.SPK, 998 * 1e18),
                fetchKyberSwapPrice(addresses.SPK, addresses.USDT, 14998 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 998 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 14998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('spk-buy-alert'),
            sell: document.getElementById('spk-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        const isBuyAlert = element.parentElement.id === 'spk-buy-alert';

const TUT = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'tut-audio-enable-btn';
        ...
        enableButton.addEventListener('click', async () => {
          ...
          const section = document.getElementById('tut-buy-alert').closest('.token-section');
  ...
  // Keep the rest of the code the same as previous version
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/TUT_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.TUT, 599 * 1e18),
                fetchKyberSwapPrice(addresses.TUT, addresses.USDT, 19999 * 1e18)
            ]);
        
            return {
                buyPrice: buyAmount ? 599 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 19999 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('tut-buy-alert'),
            sell: document.getElementById('tut-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
      ...
      if (shouldPlaySound && audioEnabled && element.id === 'tut-buy-alert') {
          playSystemAlert();
      }
  }

...
function createAudioEnableButton(tokenId) {
    ...
    async function fetchMexcContractPrice() {
        ...
        const url = 'https://contract.mexc.com/api/v1/contract/depth/NODE_USDT';
    ...
    async function fetchMexcSpotPrice() {
        const proxyUrl = 'https://api.codetabs.com/v1/proxy/?quest=';
        const url = 'https://api.mexc.com/api/v3/depth?symbol=NODEUSDT&limit=5';
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            NODE: '0x2f714d7b9a035d4ce24af8d9b6091c07e37f43fb'
        };

        try {
            // Format amounts properly without scientific notation
            const buyAmount = "198000000000000000000"; // 198 USDT with 18 decimals
            const sellAmount = "4998000000000000000000"; // 4998 NODE with 18 decimals
            
            const [buyResponse, sellResponse] = await Promise.all([
                fetch(`https://aggregator-api.kyberswap.com/bsc/api/v1/routes?tokenIn=${addresses.USDT}&tokenOut=${addresses.NODE}&amountIn=${buyAmount}`),
                fetch(`https://aggregator-api.kyberswap.com/bsc/api/v1/routes?tokenIn=${addresses.NODE}&tokenOut=${addresses.USDT}&amountIn=${sellAmount}`)
            ]);
            ...
            return {
                ...
                sellPrice: sellData.data?.routeSummary?.amountOut ? 
                    (parseFloat(sellData.data.routeSummary.amountOut) / 1e18) / 4998 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            kyberBuy: document.getElementById('node-kyber-buy-alert'),
            kyberSell: document.getElementById('node-kyber-sell-alert'),
            mexcBuy: document.getElementById('node-mexc-buy-alert'),
            mexcSell: document.getElementById('node-mexc-sell-alert')
        };
    ...
    (function init() {
        ...
        setTimeout(() => {
            if (!audioEnabled) {
                // NEW: Check only within NODE's token section
                const section = document.getElementById('node-kyber-buy-alert')?.closest('.token-section') || 
                                document.getElementById('node-buy-alert')?.closest('.token-section');
                
                // Create button only if section exists and has no audio button
                if (section && !section.querySelector('.audio-btn-container')) {
                    createAudioEnableButton('node');
                }
            }
        }, 5000);
    })();

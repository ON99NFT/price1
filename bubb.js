const BUBB = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'bubb-audio-enable-btn';
        ...
        const section = document.getElementById('bubb-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BUBB_USDT');
    ...
    async function fetchKyberPrice() {
        ...
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BUBB, 1099 * 1e18),
                fetchKyberSwapPrice(addresses.BUBB, addresses.USDT, 129999 * 1e18)
            ]);

            return {
                buyPrice: buyAmount ? 1099 / (buyAmount / 1e18) : null,
                sellPrice: sellAmount ? (sellAmount / 1e18) / 129999 : null
            };
        } catch (error) {
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('bubb-buy-alert'),
            sell: document.getElementById('bubb-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'bubb-buy-alert') {
            playSystemAlert();
        }
    }

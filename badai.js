const BADAI = (() => {
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BADAI_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            BADAI: '0x36f5675029e129b5fcabb29ec750ed268520acf7'
        };
    
        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BADAI, 2700 * 1e18),
                fetchKyberSwapPrice(addresses.BADAI, addresses.USDT, 50000 * 1e18)
            ]);
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('badai-buy-alert'),
            sell: document.getElementById('badai-sell-alert')
        };

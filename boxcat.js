const BOXCAT = (() => {
    ...
    function handleAudioInitialization() {
        ...
        enableButton.id = 'boxcat-audio-enable-btn';
        ...
        const section = document.getElementById('boxcat-buy-alert').closest('.token-section');
    ...
    async function fetchMexcPrice() {
        try {
            ...
            const response = await fetch(proxyUrl + 'https://contract.mexc.com/api/v1/contract/depth/BOXCAT_USDT');
    ...
    async function fetchKyberPrice() {
        const addresses = {
            ...
            BOXCAT: '0x63eE66898ed6Fbe0116c27dFe44E18F0cB5bF963'
        };

        try {
            const [buyAmount, sellAmount] = await Promise.all([
                fetchKyberSwapPrice(addresses.USDT, addresses.BOXCAT, 798 * 1e18),
                fetchKyberSwapPrice(addresses.BOXCAT, addresses.USDT, 12498 * 1e18)
            ]);
    ...
    async function updateAlerts() {
        const elements = {
            buy: document.getElementById('boxcat-buy-alert'),
            sell: document.getElementById('boxcat-sell-alert')
        };
    ...
    function applyAlertStyles(element, value) {
        ...
        if (shouldPlaySound && audioEnabled && element.parentElement.id === 'boxcat-buy-alert') {
            playSystemAlert();
        }
    }
